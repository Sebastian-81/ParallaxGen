...
TEST_P(BethesdaDirectoryTest, Path) {
  ...
  EXPECT_TRUE(BD->getPathLower({L"Textures\\Texture"}).wstring() == L"textures\\texture");
  EXPECT_TRUE(BD->getPathLower({"Textures\\Texture"}).string() == "textures\\texture");
  ...
  EXPECT_TRUE(BD->pathEqualityIgnoreCase({"Textures/Texture"}, {"textures/texture"}));
  EXPECT_FALSE(BD->pathEqualityIgnoreCase({"Textures/Texture1"}, {"textures/texture2"}));
  EXPECT_FALSE(BD->pathEqualityIgnoreCase({"textures/texture1"}, {"textures/texture2"}));
}

TEST_P(BethesdaDirectoryTest, Glob) {
    std::vector<wstring> GlobList{L"textures/texture*.dds", L"meshes/*.nif"};
    EXPECT_TRUE(BD->checkGlob(L"textures/texture0.dds", GlobList));
    EXPECT_TRUE(BD->checkGlob(L"meshes/mesh0.nif", GlobList));
    EXPECT_TRUE(BD->checkGlob(L"meshes/architecture/mesh1.nif", GlobList));
    EXPECT_FALSE(BD->checkGlob(L"textures/architecture/texture1.dds", GlobList));
    EXPECT_FALSE(BD->checkGlob(L"scripts/script1.pex", GlobList));
    EXPECT_FALSE(BD->checkGlob(L"textures/texture0.png", GlobList));
...
TEST_P(BethesdaDirectoryTest, Files) {
  ...
  const std::filesystem::path WRCarpet01Path{"textures/architecture/whiterun/wrcarpet01.dds"};
  ...
  EXPECT_TRUE(FileMap.at(Bridge01Path).BSAFile->Path.filename() == std::filesystem::path{"Skyrim - Textures5.bsa"});
  ...
  // Skyrim - Misc.bsa
  const std::filesystem::path AbForSwornScriptPath{"scripts/abforswornbriarheartscript.pex"};
  ...
  EXPECT_TRUE(FileMap.at(AbForSwornScriptPath).BSAFile->Path.filename() == std::filesystem::path{"Skyrim - Misc.bsa"});
  ...
  const std::filesystem::path StoneQuarry01Path{"textures/_byoh/clutter/resources/stonequarry01.dds"};
  ...
  EXPECT_TRUE(FileMap.at(StoneQuarry01Path).BSAFile->Path.filename() == std::filesystem::path{"unofficial skyrim special edition patch - textures.bsa"});
  ...
  // Skyrim - Meshes1.bsa
  const std::filesystem::path Road3way01Path{"meshes/landscape/roads/road3way01.nif"};
  ...
  EXPECT_TRUE(FileMap.at(Road3way01Path).BSAFile->Path.filename() == std::filesystem::path{"Skyrim - Meshes1.bsa"});
  ...
  const std::filesystem::path AlduinPath{"textures/actors/alduin/alduin.dds"};
  ...
  //lower upper
  //  Skyrim - Textures5.bsa from env
  const std::filesystem::path Bridge01PathUpper{"TEXTURES/LANDSCAPE/ROADS/bridge01.dds"};
